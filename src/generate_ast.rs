...
fn define_type(file: &mut std::fs::File, class_name: &str, field_list: &str) -> std::io::Result<()> {
    write!(file, "    {}(", class_name)?;
    let fields = field_list.split(", ").collect::<Vec<&str>>();
    let fields = fields.iter().map(|field| field.split(": ").collect::<Vec<&str>>()[1].trim()).collect::<Vec<&str>>();
...
fn define_visitor(file: &mut std::fs::File, base_name: &str, types: Vec<&str>) -> std::io::Result<()> {
    ...
    for type_ in &types {
        let type_name = type_.split(';').collect::<Vec<&str>>()[0].trim();
        let fields = type_.split(';').collect::<Vec<&str>>()[1].trim();
        let fields = fields.split(", ").map(|field| field.split(": ").collect::<Vec<&str>>()[0].trim()).collect::<Vec<&str>>().join(", ");

        writeln!(file, "            {}::{}({}) => {{", base_name, type_name, &fields)?;
        writeln!(file, "                visitor.visit_{}_{}({})", type_name.to_lowercase(), base_name.to_lowercase(), fields)?;
        writeln!(file, "            }},")?;
    }
